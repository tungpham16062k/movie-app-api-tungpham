...
class MovieController {
    ...
    async getOneBySlug(req, res, next) {
        try {
            ...
            const movie = await Movie.find({ slug: slug }).populate('author', 'name').populate('category', 'name').populate({ path: 'cast.actor', select: 'name avatar slug' }).populate({ path: 'director', select: 'name slug' });
            res.status(200).json({
                status: 'Successful',
                data: movie
            });
